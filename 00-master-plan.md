# HabbitAt — Master Plan & Agent Index
...
HabbitAt is an AI-powered habit enforcer for personal use: you log a habit, it schedules escalating reminders, you prove completion with a photo, an AI verifies it, and a calendar of photographic evidence becomes your record of discipline. A companion Mac app mirrors that record on the desktop.

## 2. Two apps, one system
- **HabbitAt (Android, Kotlin)** — the primary app. Habit creation, scheduling, camera proof, AI verification, calendar history, notifications.
- **HabbitAt Sync (macOS, Swift/SwiftUI)** — a real native app (menu bar + full window), not a script. Pulls the same Drive-backed data and renders the same calendar/streak experience on the Mac, read-first, with a small set of desktop-only conveniences.
...
6. **Phase 6 — HabbitAt Sync (macOS app)**: native Mac app consuming the same Drive contract, mirroring the design system, read-first with optional desktop conveniences.
