...
| `GOOGLE_DRIVE_FOLDER_NAME` | Root Drive folder name for the sync contract | `habitAt` (fixed default per the folder contract in Phase 5 — not a secret, but kept here for one source of truth) |

## macOS (`/mac`, Xcode config / gitignored plist)
| Variable | Purpose | Value |
|---|---|---|
| `GOOGLE_OAUTH_CLIENT_ID_MAC` | Read-only Drive access for habitAt Sync (Phase 6) — may be a separate OAuth client from the Android one since it's a different platform registration in Google Cloud Console | *(blank — Param provides)* |
